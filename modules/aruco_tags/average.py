...
#         cv.putText(frame, vel_str, (mx + 8, my + 22),
...
# edge to edge pixel to cm + averaged velocity in m/s
#############
## Imports ##
#############
import cv2 as cv
...
# ---------------- VELOCITY SETUP ----------------
VELOCITY_EXCLUDE = {2, 3}      # calibration tags
VELOCITY_WINDOW = 15           # sliding window size

prev_states = {}               # marker_id -> (x, y, t)
velocity_history = {}          # marker_id -> [v1, v2, ...]
...
while True:
    ...
    if ids is not None and len(ids) >= 1:
        ...
        for i in range(len(ids)):
            ...
            ###########################################
            ## STEP 1.5 — VELOCITY (AVERAGED)
            ###########################################
            if marker_id not in VELOCITY_EXCLUDE and marker_id in prev_states:
                x_prev, y_prev, t_prev = prev_states[marker_id]
                dt = now - t_prev
                ...
                if dt > 0:
                    ...
                    if cm_per_pixel is not None:
                        ...
                        if marker_id not in velocity_history:
                            velocity_history[marker_id] = []

                        velocity_history[marker_id].append(vel)

                        if len(velocity_history[marker_id]) > VELOCITY_WINDOW:
                            velocity_history[marker_id].pop(0)

                        avg_vel = sum(velocity_history[marker_id]) / len(velocity_history[marker_id])

                        print(f"[VEL] Marker {marker_id}: {avg_vel:.3f} m/s "
                              f"(avg over {len(velocity_history[marker_id])})")

                        velocities_to_draw.append(
                            (int(mid_x), int(mid_y), f"{avg_vel:.3f} m/s")
                        )

            # Update previous state for ALL markers
            prev_states[marker_id] = (mid_x, mid_y, now)
